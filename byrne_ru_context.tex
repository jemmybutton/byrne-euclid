...
\marginNote{Этими тремя положениями описываются разрешенные к~использованию инструменты: \emph{линейка} и~\emph{циркуль}. При этом линейка не имеет делений, то есть ее нельзя использовать для измерения расстояний. А~циркуль пригоден только для построения окружностей, и ни для чего больше.}
...
\drawCurrentPictureInMargin
\problemNP[3]{Е}{сли}{две прямых линии \drawUnitLine{AB} и~\drawUnitLine{CD} пересекаются, вертикальные углы \drawAngle{BEC} и~\drawAngle{AED}, \drawAngle{CEA} и~\drawAngle{DEB} будут равны между собой.}
...
\drawCurrentPictureInMargin
\problemNP[3]{Е}{сли}{у двух треугольников по две стороны соответственно равны друг другу ($\drawUnitLine[9mm]{AB} = \drawUnitLine[9mm]{EF}$ и~$\drawUnitLine[9mm]{AD} = \drawUnitLine[9mm]{GE}$), и~угол заключенный ними в~одном
\drawFromCurrentPicture{
startAutoLabeling;
...
\startCenterAlign
Сделаем $\drawFromCurrentPicture[middle][angleBAC]{
startAutoLabeling;
...
\drawCurrentPictureInMargin
\problemNP[3]{Е}{сли}{прямая \drawUnitLine{GH}, пересекая две другие прямые \drawUnitLine{CD} и~\drawUnitLine{AB}, образует накрестлежащие углы \drawAngle{CFG}, \drawAngle{HEB} и~\drawAngle{GFD} и~\drawAngle{AEH} равные между собой, то две пересекаемые прямые параллельны.}
...
к каждой добавим $\drawSizedLine{AD}^2$, тогда\\
$\drawSizedLine{BD,DC}^2 + \drawSizedLine{BD}^2 + \drawSizedLine{AD}^2 = 2 \cdot \drawSizedLine{BD,DC} \cdot \drawSizedLine{BD} + \drawSizedLine{DC}^2 + \drawSizedLine{AD}^2$\\
