...
{\tfxx Эта редакция и перевод книги Оливера Бирна \quotation{The first six books of the Elements of Euclid} подготовлены Сергеем Слюсаревым и распространяются под лицензией CC-BY-SA 4.0}
\stopnarrower
...
Язык целиком состоит из знаков, и те знаки лучшие, какие выполняют свое назначение с наибольшей точностью и быстротой. Так, для всех обычных целей применяются слышимые знаки, называемые словами, которые считаются слышимыми, доносятся ли они непосредственно через уши или опосредованно через глаза в виде слов. Геометрические построения~— это не знаки, но принадлежности геометрической науки, задача которых~— показать относительные размеры своих частей, с помощью процесса рассуждения, называемого демонстрацией. Такое рассуждение чаще всего доносилось в виде слов, букв и черных, нераскрашенных диаграмм, но поскольку испольpование цветных символов, знаков и диаграмм в искусствах и науках делает ход рассуждения более точным, а усвоение более скорым, в данном случае они были соответствующим образом внедрены.

Скорость этого весьма заманчивого способа донесения знаний такова, что «Начала» Евклида могут быть усвоены менее чем за треть от времени, какое обычно требуется, а сохранение в памяти, напротив, намного более длительное. Эти факты были установлены во множестве экспериментов, проведенных как автором, так и другими, внедрившими его задумку. Рецепт же короток и очевиден: буквы, присвоенные точкам, линиям или другим частям построения, в действительности всего лишь произвольные имена и представляют те в демонстрации, вместо этого части, получающие различные цвета, называют сами себя, так как их формы и соответствующие цвета представляют их в демонстрации.
...
\defineNewPicture{
	...
	C = whatever[A, A shifted (1,0)] = whatever[B, B shifted dir(-145+90)];
		byAngleDefineWithName(A, B, C, byyellow, 0)(B);
		byAngleDefineWithName(B, C, A, byblue, 0)(C);
		byAngleDefineWithName(C, A, B, byred, 0)(A);
		draw byNamedAngleResized();
...
Цвета и раскрашенные построения могут, на первый взгляд, показаться неуклюжим способом донесения правильных представлений о свойствах и частях математических фигур и величин, однако, как мы увидим, они являются средством более изящным и емким, чем любые предложенные ранее.
...
Точкой называют то, у чего есть положение, но нет величины, или точка — это одно лишь положение, лишенное длины, ширины и толщины. Следующее описание, возможно, более пригодно для объяснения сущности математической точки тем, кто еще не освоил идею, имеющую приведенное выше пространное определение.
...
Линия суть длина без ширины. С~помощью цветов почти так же, как и выше, понятие линии можно донести следующим образом:
...
Теперь в \drawLine{BC,BE,CE} и \drawLine{BC,BD,CD}\\
...
\startalignment[middle]
\emph{Заменив буквами диаграммы}
\stopalignment
...
\startalignment[last]
Плоская поверхность есть та, которая равно расположена по отношеню к прямым на ней.
\stopalignment
\stopDefinitionOnlyNumber

\startDefinitionOnlyNumber[reference=def:VIII]
\startalignment[last]
Плоский угол есть наклонение друг к другу двух линий, в плоскости встречающихся друг с другом, но не расположенных по одной прямой.
\stopalignment
\stopDefinitionOnlyNumber
...
\defineNewPicture{
	...
	B := (u, u);
	C := (3/2u, ypart(A));
		byAngleDefineWithName(B, A, C, byyellow, 0)(A);
		draw byNamedAngleResized();
		byLineDefine(A, B, byblue, 0, 0);
		byLineDefine(A, C, byred, 0, 0);
...
\startalignment[last]
Прямолинейный угол есть наклонение друг к другу двух пересекающихся и не совпадающих прямых линий.
\stopalignment
\stopDefinitionOnlyNumber
...
\defineNewPicture{
	...
	C := (0, u);
	D := (-4/3u, 0);
		byAngleDefine(B, A, C, black, 1);
		byAngleDefine(D, A, C, black, 1);
		draw byNamedAngleResized();
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
Когда прямая восстановленная на другой прямой образует равные смежные углы, каждый из этих углов называется \emph{прямым углом}, а каждая из этих прямых называется \emph{перпендикуляром} к другой.
\stopalignment
\stopDefinitionOnlyNumber
...
\defineNewPicture{
	...
	B := (-u, u);
	C := (3/2u, ypart(A));
		byAngleDefineWithName(B, A, C, byred, 0)(A);
		draw byNamedAngleResized();
		byLineDefine(A, B, byyellow, 0, 0);
...
\defineNewPicture{
	...
	B := (u, u);
	C := (3/2u, ypart(A));
		byAngleDefineWithName(B, A, C, byblue, 0)(A);
		draw byNamedAngleResized();
		byLineDefine(A, B, byyellow, 0, 0);
		byLineDefine(A, C, byred, 0, 0);
		draw byNamedLineSeq(0)(AC,AB);
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Круг} есть плоская фигура, содержащаяся внутри одной линии, называемой окружностью, в которой есть точка, все прямые падающие на окружность из которой равны.
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Сегментом круга} называется фигура, содержащаяся между прямой линией и частью окружности, ей отсекаемой.
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Четырехсторонняя фигура} та, у которой четыре стороны. Прямые линии  \drawUnitLine{AD} и \drawUnitLine{CB}, соединяющие вершины противоположных углов четырехсторонней фигуры называются \emph{диагоналями.}
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Равносторонним} называется треугольник, все стороны которого равны.
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
Треугольник у которого равны две стороны называется \emph{равнобедренным.}
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
Треугольник у которого есть прямой угол называется \emph{прямоугольным}.
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Тупоугольным} называется треугольник, у которого есть тупой угол.
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Остроугольным} называется треугольник, все углы которого острые.
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
Из четырехсторонних фигур, \emph{квадратом} называется та, все стороны которой равны между собой, и все углы прямые.
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Ромбом} называется четырехсторонняя фигура, все стороны которой равны, но углы не прямые.
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Прямоугольник} есть четырехугольник, все углы которого прямые, но не все стороны равны.
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Параллелограмм} есть фигура, протвоположные стороны которой равны, но ни все стороны равны, ни все углы прямые.
\stopalignment
\stopDefinitionOnlyNumber

\startDefinitionOnlyNumber[reference=def:XXXIV]
\startalignment[last]
Все прочие прямоугольники называются \emph{неправильными.}
\stopalignment
\stopDefinitionOnlyNumber
...
\drawCurrentPictureInMargin[inside]
\startalignment[last]
\emph{Параллельными} называются такие прямые линии, которые находясь в одной плоскости и будучи продолженными в обе стороны неограниченно не встречаются.
\stopalignment
\stopDefinitionOnlyNumber
\stopsupersection
...
Если из неравных величин вычитаются  неравные, остатки будут неравными.
\stopAxiomOnlyNumber
...
\defineNewPicture{
	...
	H = whatever[C, D] = whatever[E, F];
		byAngleDefineWithName(B, G, E, byred, 0)(G);
		byAngleDefineWithName(D, H, F, byyellow, 0)(H);
		draw byNamedAngleResized();
		draw byLine(A, B, byblue, 0, 0);
...
\kerncharacters[-0.01]{Углы справедливо называть четвертым видом величин. Угловые величины очевидно состоят из частей и, следовательно, их нужно признать разновижностью количеств. Ученик не должен считать, что величина угла зависит от длин прямых линий, заключающих его, и мерой взаимного расхождения которых угол является мерой. \emph{Вершиной} угла называется точка в которой встречаются \emph{стороны} угла, как, например, A.}
...
Полная длина линии или линий, заключающих фигуру называется ее \emph{периметром}. Первые шесть книг Евклида касаются только плоских фигур. Линия, проведенная из центра круга к окружности называется \emph{радиусом}. Сторона прямоугольного треугольника противолежащая прямому углу называется \emph{гипотенузой}. % oblong=rectangle, по русски это не нужно
...
\emph{Задача} это предложение, в котором предлагается что-дибо сделать, например провести линию, соответствующую данным условиям, описать круг, построить фигуру и т. п.
...
\emph{Примечание} это заметка или наблюдение, касающееся предложение, не содержащее умозаключений достаточной важности, чтобы называться \emph{следствием}.
...
\symb{$=$}
 обозначает слово \emph{равно}. Знак равенства можно читать как \emph{равно}, или \emph{равны}, но род и число не влияют на геометрическую строгость.

\symb{$\neq$}
 обозначает то же, как если бы было написано \emph{на равно}.
...
\symb{$\times$}
 этот символ обозначает произедение двух или более чисел, будучи помещенным между ними в арифметике или алгебре. Но в геометрии он обычно используется для обозначения \emph{прямоугольника}, когда помещен между \quotation{двумя прямыми линиями, содержащими один его прямых углов}. \emph{Прямоугольник} может также обозначаться точкой между двумя смежными сторонами.
...
F := C shifted d;
byAngleDefineWithName(B, A, C, byyellow, 0)(A);
byAngleDefineWithName(A, B, C, byblue, 0)(B);
byAngleDefineWithName(B, C, A, byred, 0)(C);
draw byNamedAngleResized(A, B, C);
byLineDefine(A, B, byred, 0, 0);
byLineDefine(B, C, black, 0, 0);
byLineDefine(C, A, byblue, 0, 0);
draw byNamedLineSeq(0)(CA,BC,AB);
byAngleDefineWithName(E, D, F, byyellow, 0)(D);
byAngleDefineWithName(D, E, F, byblue, 0)(E);
byAngleDefineWithName(E, F, D, byred, 0)(F);
draw byNamedAngleResized(D, E, F);
...
но $\drawAngle{BCA,DCB} = \drawAngle{ABC,CBE}$, $\therefore \drawAngle{BCA} = \drawAngle{ABC}$ \inax[ax:III].
\stopCenterAlign
...
D := A shifted (7/4u, 3u);
C := 2/3[A, D];
byAngleDefineWithName(B, A, D, byyellow, 0)(A);
byAngleDefineWithName(A, B, D, black, 0)(B);
draw byNamedAngleResized();
byLineDefine(B, C, byyellow, 0, 0);
byLineDefine(A, B, byred, 0, 0);
...
E := B shifted d;
F := C shifted d;
byAngleDefineWithName(F, D, E, black, 0)(D);
byAngleDefineWithName(C, A, B, black, 0)(A);
draw byNamedAngleResized();
byLineDefine(A, B, byred, 0, 0);
byLineDefine(B, C, black, 0, 0);
byLineDefine(C, A, byblue, 0, 0);
...
G := 4/3[B, C];
byAngleDefineWithName(B, A, C, byblue, 0)(A);
byAngleDefineWithName(C, B, A, black, 0)(B);
byAngleDefine(A, E, B, byyellow, 0);
byAngleDefine(D, E, C, byyellow, 0);
byAngleDefine(E, C, D, black, 0);
byAngleDefine(G, C, A, byred, 0);
byAngleDefine(D, C, F, black, 1);
draw byNamedAngleResized();
...
stopAutoLabeling;
} будет больше любого из противолежащих ему внутренних углов \drawAngle{B} или \drawAngle{A}.
}
...
D := C shifted (u, 0);
byAngleDefineWithName(B, A, C, byblue, 0)(A);
byAngleDefineWithName(A, B, C, black, 0)(B);
...
C := A shifted (3/2u, 2u);
D := 2[C, A];
byAngleDefineWithName(C, A, B, byblue, 0)(A);
byAngleDefine(A, B, C, black, 0);
byAngleDefine(D, B, A, byred, 0);
byAngleDefineWithName(B, D, A, byyellow, 0)(D);
draw byNamedAngleResized();
draw byLine(A, B, byyellow, 0, 0);
byLineDefine(A, C, byred, 0, 0);
byLineDefine(B, C, byblue, 0, 0);
...
C := A shifted (u, 3u);
byAngleDefineWithName(C, A, B, byblue, 0)(A);
byAngleDefineWithName(A, B, C, byred, 0)(B);
draw byNamedAngleResized();
byLineDefine(A, B, black, 0, 0);
byLineDefine(B, C, byblue, 0, 0);
byLineDefine(C, A, byred, 0, 0);
...
D := 1/2[1/2[A, B], C];
E = whatever[A, D] = whatever[B, C];
byAngleDefineWithName(B, D, A, byred, 0)(D);
byAngleDefineWithName(B, E, D, byblue, 0)(E);
byAngleDefineWithName(B, C, A, byyellow, 0)(C);
draw byNamedAngleResized();
byLineDefine(B, D, byyellow, 0, 0);
byLineDefine(A, D, black, 0, 0);
...
J := D shifted d;
byAngleDefineWithName(B, A, C, byred, 0)(A);
byAngleDefineWithName(G, F, H, byblue, 0)(F);
draw byNamedAngleResized();
...
F := B shifted d;
G := C shifted d;
byAngleDefine(B, A, C, black, 2);
byAngleDefine(C, A, D, black, 3);
byAngleDefine(F, E, G, black, 2);
...
F := C shifted d;
byAngleDefineWithName(B, A, C, byyellow, 0)(A);
byAngleDefineWithName(E, D, F, black, 0)(D);
draw byNamedAngleResized();
byLineDefine(A, B, byblue, 0, 0);
byLineDefine(B, C, black, 0, 0);
byLineDefine(C, A, byred, 0, 0);
...
G := 3/4[D, F];
byAngleDefineWithName(B, A, C, byyellow, 0)(A);
byAngleDefineWithName(C, B, A, byred, 0)(B);
byAngleDefineWithName(A, C, B, black, 1)(C);
draw byNamedAngleResized(A, B, C);
byLineDefine(A, B, byblue, 0, 0);
byLineDefine(B, C, black, 0, 0);
byLineDefine(C, A, byred, 0, 0);
draw byNamedLineSeq(0)(CA,BC,AB);
byAngleDefineWithName(E, D, F, byyellow, 0)(D);
byAngleDefine(G, E, D, black, 0);
byAngleDefine(F, E, G, byblue, 0);
byAngleDefineWithName(D, F, E, black, 1)(F);
draw byNamedAngleResized(D, GED, FEG, F);
...
но $\drawAngle{B} = \drawAngle{GED,FEG}$ (гип.)
...
byAngleDefineWithName(B, A, C, byyellow, 0)(A);
byAngleDefineWithName(C, B, A, byred, 0)(B);
draw byNamedAngleResized(A, B);
byLineDefine(A, B, byblue, 0, 0);
byLineDefine(B, C, black, 0, 0);
byLineDefine(C, A, byred, 0, 0);
draw byNamedLineSeq(0)(CA,AB,BC);
byAngleDefineWithName(F, D, E, byyellow, 0)(D);
byAngleDefineWithName(F, G, D, black, 0)(G);
byAngleDefineWithName(F, E, D, byred, 0)(E);
draw byNamedAngleResized(D, G, E);
...
byAngleDefine(A, E, F, byyellow, 0);
byAngleDefine(F, E, B, byred, 0);
byAngleDefine(C, F, E, byblue, 0);
byAngleDefine(E, F, D, byyellow, 0);
draw byNamedAngleResized();
...
Если это возможно, пусть они не будут параллельны, но сойдутся, если их продолжить; тогда внешний угол \drawAngle{FEB} будет больше \drawAngle{CFE} \inprop[prop:I.XVI], но они равны (гип.), что невозможно. Таким же образом можно показать, что они не сойдутся с другой стороны; $\therefore$ они параллельны.
...
G := 7/4[F, E];
H := 4/3[E, F];
byAngleDefine(G, E, A, black, 0);
byAngleDefine(B, E, G, byyellow, 0);
byAngleDefine(A, E, F, byred, 0);
...
byAngleDefine(F, E, I, byyellow, 0);
byAngleDefine(B, E, F, black, 0);
byAngleDefine(G, E, B, byred, 0);
byAngleDefine(E, F, D, black, 0);
draw byNamedAngleResized();
...
J := 3/2[H, G];
K := 5/4[G, H];
byAngleDefineWithName(B, G, J, byyellow, 0)(G);
byAngleDefineWithName(D, I, J, byblue, 0)(I);
byAngleDefineWithName(F, H, J, byred, 0)(H);
draw byNamedAngleResized();
draw byLine(A, B, byred, 0, 0);
draw byLine(C, D, byyellow, 0, 0);
draw byLine(E, F, byblue, 0, 0);
...
E := 4/5[A, B];
F := 1/5[C, D];
byAngleDefineWithName(F, E, A, byyellow, 0)(E);
byAngleDefineWithName(E, F, D, byred, 0)(F);
draw byNamedAngleResized();
draw byLine(E, F, black, 0, 0);
...
byAngleDefine(A, D, B, byred, 0);
byAngleDefineWithName(A, C, D, black, 0)(C);
byAngleDefineWithName(D, B, A, black, 0)(B);
draw byNamedAngleResized();
draw byLine(A, D, black, 0, 0);
byLineDefine(A, B, byred, 0, 0);
byLineDefine(C, D, byred, 1, 0);
...
byAngleDefineWithName(B, A, C, byred, 0)(A);
byAngleDefineWithName(E, B, D, byblue, 0)(B);
byAngleDefineWithName(A, E, C, black, 0)(E);
byAngleDefineWithName(A, F, D, white, 0)(F);
draw byNamedAngleResized();
byLineDefine(A, C, byblue, 0, 0);
byLineDefine(B, D, byred, 0, 0);
byLineDefine(C, D, black, 0, 0);
...
\drawCurrentPictureInMargin
\problemNP{Т}{реугольники}{\drawPolygon[bottom][polygonBCD]{BCD} и \drawPolygon[bottom][polygonEGH]{EGH} на равных основаниях и между теми же параллельными прмыми равны.}
...
byAngleDefineWithName(B, E, D, byblue, 0)(E);
byAngleDefineWithName(H, I, J, byyellow, 0)(I);
angleStandalone.I := 1;
draw byNamedAngleResized();
...
byAngleDefineWithName(A, F, E, byblue, 0)(F);
byAngleDefineWithName(F, E, I, byred, 0)(E);
byAngleDefineWithName(E, I, D, black, 0)(I);
byAngleDefineWithName(M, N, O, byyellow, 0)(N);
angleStandalone.N := 1;
draw byNamedAngleResized();
draw byLine(B, E, byred, 0, 0);
...
\drawCurrentPicture
\problemNP{К}{ данной}{прямой линии \drawUnitLine{EG} приложить параллелограмм, равный данному треугольнику \drawPolygon[middle][polygonJKL]{JKL}, и с углом, равным данному прямолинейному углу \drawAngle{N}.}
...
byAngleDefineWithName(G, I, H, byyellow, 0)(I);
byAngleDefineWithName(J, K, I, black, 0)(K);
byAngleDefineWithName(L, M, K, byblue, 0)(M);
byAngleDefineWithName(N, O, P, byred, 0)(O);
angleStandalone.O := 1;
draw byNamedAngleResized();
...
C := A shifted (0, -d);
D := A shifted (d, -d);
byAngleDefineWithName(B, A, C, black, 0)(A);
byAngleDefineWithName(D, B, A, byblue, 0)(B);
byAngleDefineWithName(C, D, B, byred, 0)(D);
byAngleDefineWithName(A, C, D, byyellow, 0)(C);
draw byNamedAngleResized();
byLineDefine(A, B, byred, 0, 0);
byLineDefine(B, D, byyellow, 0, 0);
...
\startCenterAlign
Проведем $\drawUnitLine{CA} \perp \mbox{ и } = \drawUnitLine{DC}$ (\inpropL[prop:I.XI], \inpropL[prop:I.III])
...
H := 1/2[E, F];
byAngleDefineWithName(E, A, B, byyellow, 0)(A);
byAngleDefineWithName(A, B, E, byblue, 0)(B);
byAngleDefine(A, E, C, byyellow, 0);
byAngleDefine(C, E, B, byred, 0);
byAngleDefine(E, F, G, byred, 0);
byAngleDefine(D, F, B, black, 0);
draw byNamedAngleResized();
...
\defineNewPicture{
	...
	byAngleDefineWithName(C, A, D, byblue, 0)(A);
	byAngleDefineWithName(C, B, D, byyellow, 0)(B);
	draw byNamedAngleResized();
	draw byArcBE(O, b, e, r, black, 0, 0, 0, 0)(O);
...
byAngleDefineWithName(D, A, B, byblue, 0)(A);
byAngleDefineWithName(A, E, D, byyellow, 0)(E);
byAngleDefineWithName(A, B, D, black, 0)(B);
draw byNamedAngleResized();
draw byLine(D, E, black, 0, 0);
draw byLine(E, F, black, 1, 0);
...
byAngleDefineWithName(E, A, F, byblue, 0)(A);
byAngleDefine(A, F, E, black, 0);
byAngleDefine(E, F, B, byyellow, 0);
byAngleDefineWithName(F, B, E, byred, 0)(B);
draw byNamedAngleResized();
...
и $\therefore \drawUnitLine{EF} \perp \drawUnitLine{AF,FB}$ \indef[def:X]
...
\startCenterAlign
Тогда $\drawUnitLine{CE} = \drawUnitLine{EF}$ \indef[def:XV]\\
и $\drawUnitLine{CE} = \drawUnitLine{EF,FG}$ \indef[def:XV]
...
\startCenterAlign
Тогда $\drawUnitLine{CF} = \drawUnitLine{FE}$ \indef[def:XV]\\
и $\drawUnitLine{CF} = \drawUnitLine{FE,EB}$ \indef[def:XV]
...
Из центра круга проведем \drawUnitLine{EB} и \drawUnitLine{EC}, тогда $\drawUnitLine{AE} = \drawUnitLine{EB}$ \indef[def:XV] $\drawUnitLine{EF,AE} = \drawUnitLine{EF} + \drawUnitLine{EB} > \drawUnitLine{FB}$ \inprop[prop:I.XX]. Таким же образом можно показать, что \drawUnitLine{EF,AE} больше \drawUnitLine{FC}, или любой другой линии, проведенной из той же точки к окружности. 

Теперь, согласно \inprop[prop:I.XX] $\drawUnitLine{EF} + \drawUnitLine{FC} > \drawUnitLine{EC} = \drawUnitLine{FD} + \drawUnitLine{EF}$, вычтем \drawUnitLine{EF} из обеих; $\therefore \drawUnitLine{FC} > \drawUnitLine{FD}$ \inax[ax:III], и так же можно показать, что \drawUnitLine{FD} меньше любой другой линии, проведенной из той же точки к окружности. 
...
Теперь, поскольку более чем две равных прямых линии проведено к окружности из точки, не являющейся центром, две из них должны лежать по одну сторону диаметра \drawUnitLine{DH,DF,FL}, и, поскольку из точки \drawPointL[middle][DA,DF]{D}, не являющейся центром проямые линии проведены к окружности, наибольшая из них \drawUnitLine{DF,FL}, та, что проходит через центр, а \drawUnitLine{DC}, которая ближе к \drawUnitLine{DF,FL}, $> \drawUnitLine{DB}$ расположенной дальше \inprop[prop:III.VIII], но $\drawUnitLine{DC} = \drawUnitLine{DB}$ (гип.), что невозможно.
...
и $\drawUnitLine{FC} = \drawUnitLine{AF}$ \indef[def:XV],\\
и $\drawUnitLine{DG} = \drawUnitLine{AG}$ \indef[def:XV],
...
byAngleDefineWithName(E, G, C, black, 1)(G);
draw byNamedAngleResized();
...
byAngleDefineWithName(D, C, A, black, 0)(C);
byAngleDefineWithName(A, G, D, black, 1)(G);
draw byNamedAngleResized();
draw byLine(A, C, byred, 0, 0);
draw byLine(D, C, byblue, 0, 0);
...
byAngleDefineWithName(A, B, E, byyellow, 0)(B);
byAngleDefineWithName(F, D, E, byyellow, 0)(D);
byAngleDefineWithName(F, E, A, byblue, 0)(E);
draw byNamedAngleResized();
draw byLine(A, B, byblue, 0, 0);
...
byAngleDefineWithName(F, C, G, byred, 0)(C);
byAngleDefineWithName(C, G, F, byyellow, 0)(G);
draw byNamedAngleResized();
...
byAngleDefineWithName(E, C, A, byred, 0)(C);
draw byNamedAngleResized();
...
byAngleDefine(C, A, F, byyellow, 0);
byAngleDefine(C, E, F, byblue, 0);
byAngleDefineWithName(E, C, A, byyellow, 0)(C);
byAngleDefine(B, A, F, byred, 0);
byAngleDefine(B, E, F, black, 0);
byAngleDefineWithName(E, B, A, byred, 0)(B);
draw byNamedAngleResized();
draw byLine(A, C, black, 0, 1);
draw byLine(E, C, black, 0, 1);
...
byAngleDefineWithName(B, A, D, byred, 0)(A);
byAngleDefineWithName(B, E, D, byblue, 0)(E);
byAngleDefineWithName(B, F, D, byyellow, 0)(F);
draw byNamedAngleResized();
byLineDefine(B, F, byblue, 0, 0);
byLineDefine(D, F, byred, 0, 0);
...
byAngleDefineWithName(A, C, B, byyellow, 0)(C);
byAngleDefineWithName(A, D, B, byblue, 0)(D);
draw byNamedAngleResized();
...
F := point t2 of cr2;
byAngleDefineWithName(B, A, C, byred, 0)(A);
byAngleDefineWithName(B, G, C, byyellow, 0)(G);
draw byNamedAngleResized(A, G);
...
byAngleDefineWithName(E, D, F, byblue, 0)(D);
byAngleDefineWithName(E, H, F, black, 0)(H);
draw byNamedAngleResized(D, H);
...
byAngleDefineWithName(E, D, F, byred, 0)(D);
byAngleDefineWithName(E, H, F, byred, 0)(H);
draw byNamedAngleResized(D, H);
draw byLine(D, F, black, 0, 1);
draw byLine(D, E, black, 0, 1);
...
D := point t1 of cr2;
E := point t2 of cr2;
byAngleDefineWithName(A, K, B, byred, 0)(K);
draw byNamedAngleResized(K);
draw byLine(A, B, byred, 0, 0);
...
byAngleDefineWithName(D, L, E, byyellow, 0)(L);
draw byNamedAngleResized(L);
...
byAngleDefineWithName(A, D, C, byred, 0)(D);
byAngleDefineWithName(C, B, A, byyellow, 0)(B);
draw byNamedAngleResized();
draw byLine(A, B, byred, 0, 0);
draw byLine(B, C, byblue, 0, 0);
draw byLine(C, D, black, 0, 1);
...
E := (-r, -r) shifted O;
F := (r, -r) shifted O;
byAngleDefine(A, B, E, black, 1);
byAngleDefine(D, B, A, byblue, 0);
byAngleDefine(F, B, D, byyellow, 0);
byAngleDefineWithName(B, A, D, byyellow, 0)(A);
byAngleDefine(A, D, B, black, 0);
byAngleDefine(C, D, B, black, 1);
byAngleDefineWithName(D, C, B, byred, 0)(C);
draw byNamedAngleResized();
draw byLine(A, D, black, 0, 1);
draw byLine(D, C, black, 0, 1);
...
$\therefore \drawAngle{A} = \drawAngle{FBD}$ \inax[ax:III].

Теперь $\drawAngle{ABE,DBA,FBD} = \drawTwoRightAngles = \drawAngle{A} + \drawAngle{C}$ \inprop[prop:III.XXII]

$\therefore \drawAngle{ABE,DBA} = \drawAngle{C}$ \inax[ax:III], который и будет углом в накрестлежащем сегменте.
\stopCenterAlign
...
D := (r, -r) shifted G;
K := (-r, -r) shifted G;
byAngleDefine(E, A, K, black, 1);
byAngleDefine(B, A, E, byred, 0);
byAngleDefine(D, A, B, byyellow, 0);
byAngleDefineWithName(G, B, A, byred, 0)(B);
draw byNamedAngleResized(EAK,BAE,DAB,B);
draw byLine(A, B, black, 0, 0);
...
byAngleDefineWithName(D, B, F, byblue, 0)(B);
byAngleDefineWithName(F, E, D, byred, 0)(E);
draw byNamedAngleResized();
...
O := (0, 0);
byAngleDefineWithName(A, B, C, byred, 0)(B);
byAngleDefineWithName(B, C, D, byred, 0)(C);
byAngleDefineWithName(C, D, A, byred, 0)(D);
byAngleDefineWithName(D, A, B, byred, 0)(A);
draw byNamedAngleResized();
byLineDefine(A, B, byred, 0, 0);
byLineDefine(B, C, byred, 0, 0);
byLineDefine(C, D, byred, 0, 0);
...
byAngleDefineWithName(F, E, D, byblue, 0)(E);
byAngleDefineWithName(E, D, F, byred, 0)(D);
byAngleDefineWithName(D, F, E, byyellow, 0)(F);
draw byNamedAngleResized();
...
byAngleDefineWithName(M, L, N, byred, 0)(L);
byAngleDefine(D, F, E, byred, 0);
byAngleDefineWithName(L, N, M, black, 1)(N);
byAngleDefine(F, E, D, black, 1);
byAngleDefineWithName(N, M, L, byred, 1)(M);
byAngleDefineWithName(E, D, F, byred, 1)(D);
byAngleDefineWithName(K, A, L, black, 0)(A);
byAngleDefineWithName(L, C, K, black, 0)(C);
draw byNamedAngleResized();
...
r := abs(E-D);
byAngleDefineWithName(B, E, D, byred, 0)(E);
byAngleDefineWithName(D, F, B, byred, 0)(F);
byAngleDefine(E, B, D, byblue, 0);
byAngleDefine(D, B, F, byyellow, 0);
byAngleDefine(G, C, D, black, 0);
byAngleDefine(D, C, F, black, 1);
byAngleDefine(E, D, F, black, 1);
draw byNamedAngleResized();
...
C := (0, -r);
D := (r, 0);
byAngleDefineWithName(D, A, B, byyellow, 0)(A);
byAngleDefineWithName(C, D, A, black, 0)(D);
...
и $\drawUnitLine{BE} = \drawUnitLine{AE} = \drawUnitLine{DE}$ \indef[def:XV].
...
H := (-r, -r);
K := (r, -r);
byAngleDefineWithName(F, G, H, byred, 0)(G);
byAngleDefineWithName(G, H, K, byred, 0)(H);
byAngleDefineWithName(H, K, F, byred, 0)(K);
byAngleDefineWithName(K, F, G, byred, 0)(F);
byAngleDefineWithName(B, E, C, black, 0)(E);
byAngleDefineWithName(E, C, H, byyellow, 0)(C);
draw byNamedAngleResized();
...
r3 := abs(F-A);
byAngleDefineWithName(D, A, B, black, 1)(A);
byAngleDefine(C, D, A, black, 0);
byAngleDefine(B, D, C, byyellow, 0);
byAngleDefineWithName(D, C, B, byblue, 0)(C);
byAngleDefineWithName(C, B, D, byred, 0)(B);
draw byNamedAngleResized();
draw byLine(A, D, byyellow, 0, 0);
draw byLine(B, D, byblue, 0, 0);
draw byLine(C, D, byred, 0, 0);
...
draw byPolygon(A,C,D)(byyellow);
byAngleDefineWithName(A, B, C, black, -1)(B);
byAngleDefineWithName(D, E, A, black, -1)(E);
byAngleDefineWithName(B, C, A, black, -1)(Cl);
byAngleDefineWithName(A, D, E, black, -1)(Dr);
byAngleDefineWithName(B, A, C, black, -1)(Al);
byAngleDefineWithName(D, A, E, black, -1)(Ar);
byAngleDefine(C, A, D, black, 0);
byAngleDefineExtendedWithName(E, C, A, black, 1)(ECA)(byyellow);
byAngleDefineWithName(E, C, A, black, -1)(ECAb);
byAngleDefine(D, C, E, byblue, 0);
byAngleDefine(A, D, B, black, 1);
byAngleDefine(B, D, C, byred, 0);
draw byNamedAngleResized();
...
Поскольку углы \drawAngle{ECA}, \drawAngle{DCE}, \drawAngle{CAD}, \drawAngle{BDC} и \drawAngle{ADB} равны между собой, дуги, на которых они стоят, тоже равны \inprop[prop:III.XXVI], и $\therefore$ \drawUnitLine{CD}, \drawUnitLine{BC}, \drawUnitLine{AB}, \drawUnitLine{EA} и \drawUnitLine{DE} стягивающие эти дуги также равны между собой \inprop[prop:III.XXIX] и $\therefore$ пятиугольник является равносторонним, а также равноугольным, поскольку все его углы стоят на равных дугах \inprop[prop:III.XXVII].
...
byAngleDefine(C, K, F, black, 0);
byAngleDefine(F, C, K, byblue, 1);
byAngleDefine(L, C, F, byblue, 1);
byAngleDefine(F, L, C, black, 0);
byAngleDefine(D, L, F, black, 1);
draw byNamedAngleResized();
draw byLine(F, K, byblue, 0, 0);
...
byAngleDefineWithName(F, H, C, black, 0)(H);
byAngleDefine(H, C, F, byblue, 0);
byAngleDefine(F, C, K, byblue, 0);
byAngleDefineWithName(C, K, F, black, 0)(K);
byAngleDefine(K, D, F, byred, 0);
byAngleDefine(F, D, E, byred, 0);
byAngleDefine(D, E, F, black, 1);
byAngleDefine(F, E, M, black, 1);
draw byNamedAngleResized();
...
и $\drawAngle{H} = \drawAngle{K} = \mbox{ прямому углу }$;
...
byAngleDefine(D, F, C, black, 1);
byAngleDefine(B, C, F, black, 0);
...
byAngleDefine(A, G, F, black, -1);
byAngleDefine(B, G, A, black, -1);
byAngleDefine(C, G, B, black, -1);
byAngleDefine(D, G, C, byred, 0);
byAngleDefine(E, G, D, byblue, 0);
byAngleDefine(F, G, E, black, 0);
draw byNamedAngleResized();
draw byLine(D, H, byred, 0, 0);
draw byLine(A, D, black, 0, 0);
...
$\therefore \drawAngle{DGC} = \drawAngle{EGD} = \drawAngle{FGE} = \dfrac{1}{3} \drawTwoRightAngles$ \inprop[prop:I.XXXII], и вертикальные углы равны между собой \inprop[prop:I.XV], и стоят на равных дугах \inprop[prop:III.XXVI], которые стагивают равные хорды \inprop[prop:III.XXIX]. 
...
Возьмем четыре числа $\color[byred]{8}$, $\color[black]{7}$, $\color[byblue]{10}$, и $\color[byyellow]{9}$.
...
byAngleDefine(B, A, D, byyellow, 0);
byAngleDefine(D, A, C, black, 0);
byAngleDefine(C, A, E, black, 1);
byAngleDefineWithName(A, E, C, byblue, 0)(E);
byAngleDefineWithName(E, C, A, byred, 0)(C);
draw byNamedAngleResized();
...
byAngleDefineWithName(A, B, C, byyellow, 0)(B);
byAngleDefine(B, C, A, byblue, 0);
byAngleDefineWithName(C, A, B, black, 1)(A);
byAngleDefine(D, C, E, byred, 0);
byAngleDefineWithName(C, E, D, black, 0)(E);
byAngleDefineWithName(E, D, C, byred, 1)(D);
draw byNamedAngleResized();
draw byLine(C, A, byred, 0, 0);
...
byAngleDefineWithName(E, G, F, black, 0)(G);
byAngleDefine(G, E, F, byblue, 0);
byAngleDefine(E, F, G, byred, 0);
draw byNamedAngleResized(D, DEF, EFD, G, GEF, EFG);
draw byLine(E, F, black, 0, 0);
...
но $\drawUnitLine{AB} : \drawUnitLine{BC} :: \drawUnitLine{DE} : \drawUnitLine{EF}$ (гип.);\\
...
но $\drawAngle{GEF} = \drawAngle{B}$ (постр.) и $\therefore \drawAngle{DEF} = \drawAngle{B}$.\\
...
byAngleDefineWithName(A, B, C, byyellow, 0)(B);
byAngleDefineWithName(B, A, C, byblue, 0)(A);
byAngleDefineWithName(A, C, B, byred, 0)(C);
draw byNamedAngleResized(A, B, C);
byLineDefine(A, B, byred, 1, 0);
...
byAngleDefineWithName(D, E, F, byyellow, 1)(E);
byAngleDefine(E, F, D, byred, 1);
byAngleDefine(F, D, G, byblue, 0);
byAngleDefine(G, F, D, byred, 0);
byAngleDefineWithName(D, G, F, black, 0)(G);
draw byNamedAngleResized(FDE, E, EFD, FDG, GFD, G);
draw byLine(F, D, byblue, 0, 0);
byLineDefine(F, G, byyellow, 1, 0);
...
но $\drawUnitLine{BC} : \drawUnitLine{CA} :: \drawUnitLine{EF} : \drawUnitLine{FD}$ (гип.)\\
...
но $\drawAngle{GFD} = \drawAngle{A}$ (постр.) и $\therefore \drawAngle{EFD} = \drawAngle{A}$,\\
...
byAngleDefineWithName(B, C, A, byblue, 0)(C);
byAngleDefineWithName(C, A, B, byred, 0)(A);
byAngleDefine(A, B, G, black, 0);
byAngleDefine(G, B, C, black, 1);
byAngleDefine(C, G, B, byyellow, 0);
byAngleDefine(B, G, A, byred, 0);
draw byNamedAngleResized(C, A, ABG, GBC, CGB, BGA);
draw byLine(B, G, byblue, 0, 0);
byLineDefine(A, B, byyellow, 0, 0);
byLineDefine(B, C, byred, 0, 0);
byLineDefine(C, A, black, 0, 0);
draw byNamedLineSeq(0)(CA,BC,AB);
byAngleDefineWithName(D, E, F, byblue, 1)(E);
byAngleDefineWithName(E, F, D, byyellow, 1)(F);
byAngleDefineWithName(F, D, E, byred, 1)(D);
draw byNamedAngleResized(E, F, D);
...
Поскольку $\drawAngle{C} = \drawAngle{F}$ (гип.). и $\drawAngle{GBC} = \drawAngle{E}$ (постр.)\\
...
но $\drawUnitLine{BC} : \drawUnitLine{AB} :: \drawUnitLine{EF} : \drawUnitLine{DE}$ (гип.)
...
Но \drawAngle{A} меньше прямого угла (гип.)\\
...
$\therefore$ они равны, и поскольку $\drawAngle{C} = \drawAngle{F}$ (гип.)\\
...
byAngleDefineWithName(A, B, C, black, 0)(B);
byAngleDefineWithName(B, C, A, byblue, 1)(C);
byAngleDefineWithName(B, D, A, byblue, 0)(D);
...
и продлим \drawSizedLine{AD,DC} пока вся продленная линия \drawSizedLine{AD,DC,CE} не будет содержать \drawSizedLine{AD} столько раз, какую часть требуется отнять от \drawSizedLine{AF,FB}.
...
то есть \drawSizedLine{AD,DC,CE} содержит \drawSizedLine{AD} столько раз, сколько \drawSizedLine{AF,FB} содержит требуемую часть (постр.);
...
или $\drawSizedLine{GB} : \drawSizedLine{FG} :: \drawSizedLine{E'C'} : \drawSizedLine{D'E'}$ (постр.),\\
и $\drawSizedLine{FG} : \drawSizedLine{AF} :: \drawSizedLine{DE} : \drawSizedLine{AD}$ \inprop[prop:VI.II],\\
$\drawSizedLine{FG} : \drawSizedLine{AF} :: \drawSizedLine{D'E'} : \drawSizedLine{A'D'}$ (постр.),
...
но $\drawSizedLine{BD} = \drawSizedLine{AC} = \drawSizedLine{A'C'}$ (постр.);\\
...
byAngleDefineWithName(A, D, C, byblue, 0)(D);
draw byNamedAngleResized();
...
и $\therefore$ между \drawUnitLine{A} и \drawUnitLine{B} (постр.).
\stopCenterAlign
...
\drawCurrentPictureInMargin
\problemNP[2]{В}{равных}{и равноугольных параллелограммах \drawPolygon{BFAD} и \drawPolygon{CEBG},  стороны при равных углах взаимно пропорциональны ($\drawUnitLine{BG} : \drawUnitLine{BF} :: \drawUnitLine{BD} : \drawUnitLine{BE}$). И~равноугольные параллелограммы, у которых стороны взаимно пропорциональны, равны.}
...
\drawUnitLine{BD} &: \drawUnitLine{BE} \mbox{(гип.)}\cr
...
но $ \drawUnitLine{AE} : \drawUnitLine{BA} :: \drawUnitLine{AC} : \drawUnitLine{DA}$, (гип.)
...
draw byLineWithName(Fe, Fb, black, 1, 0)(F);
...
И поскольку\\
$\drawUnitLine{AB} : \drawUnitLine{CD} :: \drawUnitLine{E} : \drawUnitLine{F}$ (гип.)\\
$\therefore \drawUnitLine{AB} : \drawUnitLine{CD} :: \drawUnitLine{CH} : \drawUnitLine{AG}$ (постр.)\\
...
и $\drawUnitLine{AG} = \drawUnitLine{F}$ (постр.)\\
...
\startProposition[title={Предл. XVII. Задача},reference=prop:VI.XVIII]
\defineNewPicture{
pair C, D, E, F, L, d;
C := (0, 0);
D := (3/2u, 0);
...
byAngleDefineExtendedWithName(D, F, E, byred, 1)(F)(black);
byAngleDefineExtendedWithName(D, E, L, byyellow, 1)(E)(byred);
byAngleDefineWithName(D, C, F, byred, 1)(C);
byAngleDefine(F, D, C, byblue, 1);
byAngleDefine(E, D, F, black, 1);
byAngleDefine(L, D, E, byyellow, 1);
draw byNamedAngleResized(F, E, C, FDC, EDF, LDE);
draw byLine(D, F, byred, 1, 0);
byLineDefine(D, E, byyellow, 1, 0);
byLineDefine(C, D, black, 1, 0);
...
byAngleDefineExtendedWithName(B, G, H, black, 1)(G)(byred);
byAngleDefineExtendedWithName(B, H, K, byyellow, 1)(H)(byred);
byAngleDefineWithName(B, A, G, byred, 0)(A);
byAngleDefine(G, B, A, byblue, 0);
byAngleDefine(H, B, G, black, 0);
byAngleDefine(K, B, H, byyellow, 0);
draw byNamedAngleResized(G, H, A, GBA, HBG, KBH);
...
то, согласон \inpropL[prop:VI.IV],
...
draw byPolygon(D,E,F)(byyellow);
byAngleDefineWithName(D, E, F, black, 0)(E);
draw byNamedAngleResized(E);
byLineDefine(D, E, byred, 0, 0);
...
Но $\drawUnitLine{BG,GC} : \drawUnitLine{EF} :: \drawUnitLine{EF} : \drawUnitLine{BG}$ (постр.),\\
...
byAngleDefineWithName(B, C, D, black, 0)(C);
byAngleDefine(B, D, C, byred, 0);
draw byNamedAngleResized(BDE, C, BDC);
draw byLine(B, D, black, 1, 0);
byLineDefine(B, E, black, 0, 0);
byLineDefine(B, C, byblue, 0, 0);
byLineDefine(C, D, byblue, 1, 0);
...
byAngleDefineWithName(G, H, K, black, 1)(H);
byAngleDefine(G, K, H, byred, 1);
draw byNamedAngleResized(GKL, H, GKH);
...
Поскольку $\drawUnitLine{AB} : \drawUnitLine{CD} :: \drawUnitLine{EF} : \drawUnitLine{GH}$ (гип.)\\
$\drawUnitLine{CD} : \drawUnitLine{O} :: \drawUnitLine{GH} : \drawUnitLine{P}$ (постр.).\\
$\therefore$ по равенству,\\
...
\startCenterAlign
Оставим то же построение.\\
$\drawPolygon{AB} : \drawPolygon{CD} :: \drawPolygon{EF} : \drawPolygon{GH}$ (гип.),

$\therefore \drawUnitLine{AB} : \drawUnitLine{O} :: \drawUnitLine{EF} : \drawUnitLine{P}$ (постр.),
...
и $\drawAngle{GCE} = \drawAngle{BCD}$ (гип.),\\
...
byAngleDefineWithName(C, B, L, byred, 0)(B);
draw byNamedAngleResized(B);
pair Da, Db, Dc, Dd, De;
...
byAngleDefineWithName(F, C, E, byyellow, 0)(C);
draw byNamedAngleResized(C);
draw byLine(C, E, byred, 0, 0);
draw byLine(B, C, byyellow, 0, 0);
draw byLine(C, F, black, 1, 0);
pair K, G, H;
...
\drawCurrentPictureInMargin
\problemNP[2]{П}{остроить}{прямолинейную фигуру подобную данной \drawPolygon[middle][triangleABC]{ABC} и равную другой данной \drawPolygon[middle][polygonD]{D}.}
...
и тогда \drawUnitLine{BC} и \drawUnitLine{CF} будут лежать на одной опямой (\inpropL[prop:I.XXIX], \inpropL[prop:I.XIV]).
...
и $\drawUnitLine{BC} : \drawUnitLine{GH} :: \drawUnitLine{GH} : \drawUnitLine{CF}$ (постр.),\\
...
но $\triangleABC\ = \polygonBCEL$ (постр.),\\
и $\therefore \triangleKGH\ = \polygonCFME$ \inprop[prop:V.XIV];\\
и $\polygonCFME\ = \polygonD$ (постр.);\\
...
\drawCurrentPictureInMargin
\problemNP[2]{Е}{сли}{у подобных и подобно расположенных параллелограммов
\drawFromCurrentPicture[middle][parallelogramAEFG]{
...
но $\drawSizedLine{AG} : \drawSizedLine{AK,KE} :: \drawSizedLine{AG,GD} : \drawSizedLine{AK,KE,EB}$ (гип.),\\
...
\drawCurrentPictureInMargin
\problemNP{Р}{азделить}{данную прямую \drawSizedLine{AE,EC,CB} так, чтобы прямоугольник, заключенный между частями был равен данной площади, не провосходящей квадрат половины данной прямой.}
...
тогда $\drawSizedLine{AE,EC} > \drawSizedLine{G}$ (гип.).\\
...
Но $\drawSizedLine{DC} = \drawSizedLine{G}$ (постр.),\\
...
draw byLineWithName(H, G, black, 0, 0)(G);
...
byAngleDefineWithName(C, A, B, byyellow, 0)(A);
byAngleDefineWithName(A, B, C, byred, 0)(B);
byAngleDefine(B, C, A, byblue, 0);
byAngleDefineWithName(E, D, C, black, 0)(D);
byAngleDefine(D, C, E, black, 1);
byAngleDefine(A, C, D, byyellow, 0);
draw byNamedAngleResized();
byLineDefine(A, B, byblue, 0, 0);
...
$\therefore \drawAngle{A} = \drawAngle{D}$;\\
и поскольку $\drawUnitLine{AB} : \drawUnitLine{CA} :: \drawUnitLine{DC} : \drawUnitLine{ED}$ (гип.),\\
...
byAngleDefineWithName(E, D, F, byyellow, 1)(D);
byAngleDefineWithName(E, H, F, black, 1)(EF);
byAngleDefineWithName(F, H, M, byred, 1)(FM);
byAngleDefineWithName(M, H, N, byblue, 1)(MN);
draw byNamedAngleResized(D, EF, FM, MN);
draw byLine(D, E, black, 0, 1);
draw byLine(D, F, black, 0, 1);
...
\drawCurrentPictureInMargin
\problemNP{В}{равных}{кругах \drawCircle[middle][1/4]{G} и \drawCircle[middle][1/4]{H} углы,что в центре, что на окружности, относятся друг к другу как дуги, на которых они стоят $\drawAngle{BC} : \drawAngle{EF} ::
...
byAngleDefineWithName(C, E, B, byyellow, 0)(E);
byAngleDefine(E, B, C, byblue, 1);
byAngleDefine(C, B, D, byred, 0);
byAngleDefineWithName(B, D, C, byyellow, 1)(D);
byAngleDefine(B, C, E, black, 1);
byAngleDefine(D, C, B, byblue, 0);
byAngleDefine(F, C, D, black, 0);
draw byNamedAngleResized();
draw byLine(B, E, byred, 0, 0);
draw byLine(B, C, byyellow, 0, 0);
byLineDefine(A, B, byblue, 0, 0);
...
\drawCurrentPictureInMargin
\problemNP{Е}{сли}{угол треугольника рассекается пополам прямой линией, которая также сечет его основания, прямоугольник заключенный между сторонами треугольника равен прямоугольнику, заключенному между частями основания, сместе с квадратом прямой, рассекающей угол.}
...
Поскольку $\drawAngle{CAE} = \drawAngle{EAB}$ (гип.),\\
...
byAngleDefineWithName(A, B, C, byyellow, 1)(B);
byAngleDefineWithName(A, E, C, byred, 1)(E);
byAngleDefineWithName(B, D, A, byblue, 0)(D);
...
тогда, поскольку $\drawAngle{D} = \drawAngle{BCA,ECB}$ (постр. и \inpropL[prop:III.XXXI]);\\
и $\drawAngle{B} = \drawAngle{E}$ \inprop[prop:III.XXI];\\
...
byAngleDefineWithName(B, D, A, byblue, 1)(D);
byAngleDefineWithName(A, B, D, byyellow, 1)(B);
...
\drawCurrentPictureInMargin
\problemNP{П}{рямоугольник,}{заключенный между диагоналями четырехугольника, вписанного в круг равен прямоугольникам, заключенным между противоположными сторонами вместе взятым.}
...
Теперь, поскольку $\drawAngle{EAB} = \drawAngle{DAC}$ (постр.),\\
и $\drawAngle{B} = \drawAngle{ACD}$ \inprop[prop:III.XXI],\\
