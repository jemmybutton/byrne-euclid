...
Поскольку $\drawUnitLine{AB} = \drawUnitLine{CA}$ (\conststr),\\
\drawUnitLine{AD} общая обоим треугольникам\\
и $\drawUnitLine{CD} = \drawUnitLine{DB}$ (\conststr),
...
поскольку $\drawUnitLine{AB} = \drawUnitLine{AC}$ (\conststr),\\
...
Поскольку \inprop[prop:I.VIII], раз $\drawUnitLine{DB} = \drawUnitLine{CD}$ (\conststr),\\
\drawUnitLine{AD} общая обоим треугольникам,\\
...
$\drawUnitLine{BE} = \drawUnitLine{EC}$, $\drawAngle{AEB} = \drawAngle{DEC}$ \inprop[prop:I.XV] и $\drawUnitLine{AE} = \drawUnitLine{ED}$ (\conststr),\\
...
Поскольку $\drawUnitLine{CA} = \drawUnitLine{AD}$ (\inaxL[ax:I]. \hypstr \conststr)\\
...
но $\drawPolygon{HLEG} = \drawProportionalLine{DC}^2$ \inprop[prop:II.IV]\\
$\therefore \squareCBFE = \drawProportionalLine{HG,DH}^2 = \drawPolygon{BDHM,DCLH,CLKA,HLEG}$ (\conststr, \inaxL[ax:II])
...
Поскольку в \drawLine{AG,DG,AD} и \drawLine{DB,DG,BG} $\drawUnitLine{AG} = \drawUnitLine{BG}$ (\hypstr и \indefL[def:XV]),\\
...
но $\drawAngle{FDG,GDB} = \drawRightAngle$ (\conststr), $\drawAngle{GDB} = \drawAngle{FDG,GDB}$ \inax[ax:XI]\\
что невозможно;
\stopCenterAlign
...
Теперь, поскольку $\drawAngle{EAB} = \drawAngle{DAC}$ (\conststr),\\
