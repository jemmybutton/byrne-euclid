...
\drawCurrentPictureInMargin
\problem{I}{f}{two parallel planes be cut by any plane, their common sections are parallel.}
...
Now, since \drawUnitLine{EF,FK} is in \drawPolygon{planeAB}, therefore all the points on \drawUnitLine{EF,FK} are also in \drawPolygon{planeAB}. \byref{prop:XI.I}
...
But straight lines which do not meet in either direction are parallel. \byref{def:I.XXIII}
...
\starttheorem{Prop. XVII. Theor.}\label{prop:XI.XVII}
\defineNewPicture{
	numeric w, d;
	color s;
	w := 2cm;
...
\drawCurrentPictureInMargin
\problem{I}{f}{two straight lines be cut by parallel planes, they will be cut in the same ratios.}
...
Now, since the two parallel planes \drawPolygon{planeKL}, \drawPolygon{planeMN} are cut by the plane \drawPolygon{planeDOEB}, their common sections \drawUnitLine{EO}, \drawUnitLine{BD} are parallel. \byref{prop:XI.XVI}

For the same reason, since the two \drawPolygon{planeGH}, \drawPolygon{planeKL} are cut by the plane \drawPolygon{planeFCAO}, their common sections \drawUnitLine{AC}, \drawUnitLine{OF} are parallel. \byref{prop:XI.XVI}

And, since $\drawUnitLine{EO} \parallel \drawUnitLine{BD}$, one of the sides of the triangle \drawLine{AB,BD,DA}, therefore, proportionally, $\drawUnitLine{AE}:\drawUnitLine{EB}::\drawUnitLine{AO}:\drawUnitLine{OD}$. \byref{prop:VI.II}

Again, since $\drawUnitLine{OF} \parallel \drawUnitLine{AC}$, one of the sides of the triangle \drawLine{AD,DC,CA}, proportionally, $\drawUnitLine{AO}:\drawUnitLine{OD}::\drawUnitLine{CF}\drawUnitLine{FD}$. \byref{prop:VI.II}
...
\starttheorem{Prop. XVIII. Theor.}\label{prop:XI.XVIII}
\defineNewPicture{
	numeric w, h, d;
	color s;
	w := 3cm;
...
Let a point \drawPointL{F} be taken at random on \drawUnitLine{CE}, and from \drawPointL{F} let $\drawUnitLine{FG} \perp \drawUnitLine{CE}$ be drawn in \drawPolygon{planeDE}. \byref{prop:I.XI}

Now, since $\drawUnitLine{AB} \perp \drawPolygon{refPlane}$, \drawUnitLine{AB}  is also at right angles to all the straight lines which meet it and are in the plane of reference. \byref{def:XI.III} So that it is also at right angles to \drawUnitLine{CE};
$\therefore \drawAngle{ABE} = \drawRightAngle$.

But $\drawAngle{GFB} = \drawRightAngle$, $\therefore \drawUnitLine{AB} \parallel \drawUnitLine{FG}$. \byref{prop:I.XXVIII}

But $\drawUnitLine{AB} \perp \drawPolygon{refPlane}$, $\therefore \drawUnitLine{FG} \perp \drawPolygon{refPlane}$. \byref{prop:XI.VIII}

Now a plane is at right angles to a plane, when the straight lines drawn, in one of the planes, at right angles to the common section of the planes are at right angles to the remaining plane. \byref{def:XI.IV}
...
\starttheorem{Prop. XIX. Theor.}\label{prop:XI.XIX}
\defineNewPicture{
	numeric w, h, d;
	color s;
	w := 3cm;
...
\drawCurrentPictureInMargin
\problem{I}{f}{two planes which cut one another be at right angles to any plane, their common section will also be at right angles to the same plane.}
...
Now, since $\planeAB \perp \drawPolygon{refPlane}$, and \drawUnitLine{DE} has been drawn in the \planeAB\ at right angles to \drawUnitLine{AD}, their common section,  $\therefore \drawUnitLine{DE} \perp \drawPolygon{refPlane}$. \byref{def:XI.IV}

Similarly we can prove that $\drawUnitLine{DF} \perp \drawPolygon{refPlane}$.

Therefore from the same point \drawPointL{D} two straight lines have been set up at right angles to the plane of reference on the same side: which is impossible. \byref{prop:XI.XIII}
...
\starttheorem{Prop. XX. Theor.}\label{prop:XI.XX}
\defineNewPicture{
	numeric w, h, d;
	color s;
	w := 3cm;
...
\drawCurrentPictureInMargin
\problem{I}{f}{a solid angle be contained by three plane angles, any two, taken together in any manner, are greater than the remaining one.}
...
Now, since in \drawLine{BD,DA,AB} and \drawLine{AE,EB,BA} $\drawUnitLine{DA} = \drawUnitLine{AE}$, and \drawUnitLine{AB} is common, two sides are equal to two sides; and $\drawAngle{DAB} = \drawAngle{BAE}$, $\therefore \drawUnitLine{DB} = \drawUnitLine{BE}$. \byref{prop:I.IV}

And, since in \drawLine{BD,DC,CB} the two sides \drawUnitLine{BD}, \drawUnitLine{DC} are greater than \drawUnitLine{BC} \byref{prop:I.XX}, and of these \drawUnitLine{DB} was proved equal to \drawUnitLine{BE}, therefore the remainder \drawUnitLine{DC} is greater than the remainder \drawUnitLine{EC}.

Now, since in \drawLine{DC,CA,AD} and \drawLine{AC,CE,EA} $\drawUnitLine{DA} = \drawUnitLine{AE}$, and \drawUnitLine{AC} is common, and the base \drawUnitLine{DC} is greater than the base \drawUnitLine{EC}, $\therefore \drawAngle{DAC} > \drawAngle{EAC}$. \byref{prop:I.XXV}
...
\starttheorem{Prop. XXI. Theor.}\label{prop:XI.XXI}
\defineNewPicture{
...
\drawCurrentPictureInMargin
\problem{A}{ny}{solid angle is contained by plane angles less than four right angles.}
...
stopAutoLabeling;
} is contained by the three plane angles \drawAngle{CBA}, \drawAngle{ABD}, \drawAngle{CBD}, any two are greater than the remaining one. \byref{prop:XI.XX}
...
But $\drawAngle{CBD} + \drawAngle{BDC} + \drawAngle{BCD} = \drawTwoRightAngles$. \byref{prop:I.XXXII}
...
\starttheorem{Prop. XXII. Theor.}\label{prop:XI.XXII}
\defineNewPicture{
	numeric d, a[];
	pair s;
	d := 3/2cm;
...
\drawCurrentPictureInMargin
\problem{A}{ny}{there be three plane angles of which two, taken together in any manner, are greater than the remaining one, and they are contained by equal straight lines, it is possible to construct a triangle out of the straight lines joining the extremities of the equal straight lines.}
...
Now, since the two sides \drawUnitLine{AB}, \drawUnitLine{BC} are equal to the two sides \drawUnitLine{KH}, \drawUnitLine{HL}, and $\drawAngle{ABC} = \drawAngle{KHL}$, $\therefore \drawUnitLine{AC} = \drawUnitLine{KL}$. \byref{prop:I.IV}
...
And, since the two sides \drawUnitLine{GH}, \drawUnitLine{HL} are equal to the two sides \drawUnitLine{DE}, \drawUnitLine{EF}, and $\drawAngle{GHL} > \drawAngle{DEF}$, $\therefore \drawUnitLine{GL} > \drawUnitLine{DF}$. \byref{prop:I.XXIV}
...
\startproblem{Prop. XXIII. Prob.}\label{prop:XI.XXIII}
\defineNewPicture{
	numeric d, a[];
	pair s;
	d := 3/2cm;
...
\drawCurrentPictureInMargin
\problem{T}{o}{construct a solid angle out of three plane angles two of which, taken togetfier in any manner, are greater than the remaining one: thus the three angles must be less than four right angles.}
...
$\therefore \drawUnitLine{LM} \parallel \drawUnitLine{PQ}$ \byref{prop:VI.II}, and \drawLine{LM,MO,OL} is equiangular with \drawLine{PQ,QO,OP} \byref{prop:I.XXIX}. $\therefore \drawUnitLine{OL}:\drawUnitLine{LM}::\drawUnitLine{OP}::\drawUnitLine{PQ}$ \byref{prop:VI.IV}. And alternately, $\drawUnitLine{LO}:\drawUnitLine{OP}::\drawUnitLine{LM}::\drawUnitLine{PQ}$ \byref{prop:V.XVI}.
...
Since, then, the two sides \drawUnitLine{AB}, \drawUnitLine{BC} are equal to the two sides \drawUnitLine{PO}, \drawUnitLine{OQ}, and the base \drawUnitLine{AC} is greater than the base \drawUnitLine{PQ}, $\therefore \drawAngle{ABC} > \drawAngle{QOP}$. \byref{prop:I.XXV}
...
Let then \drawUnitLine{OR} be set up from \drawPointL{O} at right angles to the plane of \drawCircle{LMN} \byref{prop:XI.XII}. And let $\drawUnitLine{OR}^2 =\drawUnitLine{AB}^2 - \drawUnitLine{LO}^2$ (Lemma). Let \drawUnitLine{RL}, \drawUnitLine{RM}, \drawUnitLine{RN} be joined.
...
And, since $\drawUnitLine{LO} = \drawUnitLine{OM}$, while \drawUnitLine{OR} is common and at right angles, $\therefore \drawUnitLine{RL} = \drawUnitLine{RM}$. \byref{prop:I.IV}
...
But $\drawUnitLine{LR}^2 = \drawUnitLine{LO}^2, \drawUnitLine{OR}^2$, for $\drawAngle{LOR} = \drawRightAngle$ \byref{prop:I.XLVII}. $\therefore \drawUnitLine{AB}^2 = \drawUnitLine{RL}^2$, $\therefore \drawUnitLine{AB} = \drawUnitLine{RL}$.
...
And, since the two sides \drawUnitLine{LR}, \drawUnitLine{RM} are equal to the two sides \drawUnitLine{AB}, \drawUnitLine{BC}, and the base \drawUnitLine{LM} is by hypothesis equal to the base \drawUnitLine{AC}, $\therefore \drawAngle{LRM} = \drawAngle{ABC}$. \byref{prop:I.VIII}
...
\drawCurrentPictureInMargin
Let the semicircle \drawArc{O} be described on \drawUnitLine{A'B'}, and into \drawArc{O} let \drawUnitLine{A'C'} be fitted equal to \drawUnitLine{LO}, not being greater than the diameter \drawUnitLine{A'B'} \byref{prop:IV.I}. Let \drawUnitLine{C'B'} be joined.

Since then the angle \drawAngle{A'C'B'} is an angle in the semicircle \drawArc{O}, $\therefore \drawAngle{A'C'B'} = \drawRightAngle$. \byref{prop:III.XXXI}

$\therefore \drawUnitLine{A'B'}^2 = \drawUnitLine{A'C'}^2 + \drawUnitLine{C'B'}^2$. \byref{prop:I.XLVII}
...
\starttheorem{Prop. XXIV. Theor.}\label{prop:XI.XXIV}
\defineNewPicture{
	color d[];
	d1 := (5/2cm, 0cm, 0cm);
	d2 := (1/5cm, 2cm, 0cm);
...
\drawCurrentPictureInMargin
\problem{I}{f}{a solid be contained by parallel planes, the opposite planes in it are equal and parallelogrammic.}
...
their common sections are parallel. \byref{prop:XI.XVI}

$\therefore \drawUnitLine{AB} \parallel \drawUnitLine{DC}$.

Again, since the two parallel planes \planeBF, \planeAE\ are cut by the plane \planeAC, their common sections are parallel. \byref{prop:XI.XVI}
...
another, not in the same plane; therefore they will contain equal angles. \byref{prop:XI.X}

$\therefore \drawAngle{ABH} =  \drawAngle{DCF}$.

And, since the two sides \drawUnitLine{AB}, \drawUnitLine{BH} are equal to the two sides \drawUnitLine{DC}, \drawUnitLine{CF} \byref{prop:I.XXXIV}, and $\drawAngle{ABH} =  \drawAngle{DCF}$, $\therefore \drawUnitLine{AH} = \drawUnitLine{DF}$, and the triangle \drawLine{AB,BH,HA} is equal to the triangle \drawLine{DC,CF,FD}. \byref{prop:I.IV}

And the parallelogram \planeAH\ is double of the triangle \drawLine{AB,BH,HA}, and the parallelogram \planeDF\ double of the triangle \drawLine{DC,CF,FD}. \byref{prop:I.XXXIV}
...
\starttheorem{Prop. XXV. Theor.}\label{prop:XI.XXV} % this one's unfinished due to lack of enthusiasm
\defineNewPicture{
	color d[];
	numeric f;
	d1 := (-1/8cm, 3/2cm, 0cm);
...
\drawCurrentPictureInMargin
\problem{I}{f}{a parallelepipedal solid be cut by a plane which is parallel to the opposite planes, then, as the base is to the base, so will the solid be to the solid.}
...
$\pgLP = \pgKV = \pgAF$, $\drawPolygon[middle][pgKO]{LOK'K} = \drawPolygon[middle][pgKB]{KK'BA} = \drawPolygon[middle][pgAG]{ABGE}$, and further $\drawPolygon[middle][pgLX]{LOXL'} = \drawPolygon[middle][pgKQ]{KK'QP} = \pgAR$, for they are opposite. \byref{prop:XI.XXIV}
...
$\therefore \pgAF : \pgEC :: \solidAU :: \solidED$. \byref{def:V.V}
