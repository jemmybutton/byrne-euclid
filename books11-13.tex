...
Therefore \drawUnitLine{GH} is also at right angles to all the straight lines which meet it and are in \drawPolygon{planeEAD}. \inprop[def:XI.III]
...
For the same reason the angle \drawAngle{EAB} is also right; $\therefore \drawAngle{CAE} = \drawAngle{EAB}$.
